function New-SA {
    ...
    begin {
        ...
        # Check for PSToolsConfig variable required settings
        Test-PSToolsConfig -Domain $Domain
    ...
    process {
        function Initialize-AccountDetails {
            ...
            switch ($Env) {
                { 'P', 'PRD' -contains $_ } {
                    $Script:UserOU = $PSToolsConfig."$Domain".ProdServiceAccountOU
                    $Script:AcctEnv = "PRD"
                    $Script:SAGroup = $PSToolsConfig."$Domain".ProdServiceAccountGroup
                }
                { 'U', 'UAT' -contains $_ } {
                    $Script:UserOU = $PSToolsConfig."$Domain".UATServiceAccountOU
                    $Script:AcctEnv = "UAT"
                    $Script:SAGroup = $PSToolsConfig."$Domain".UATServiceAccountGroup
                }
                { 'T', 'TRN' -contains $_ } {
                    $Script:UserOU = $PSToolsConfig."$Domain".TrainingServiceAccountOU
                    $Script:AcctEnv = "TRN"
                    $Script:SAGroup = $PSToolsConfig."$Domain".TrainingServiceAccountGroup
                }
                { 'Q', 'TST' -contains $_ } {
                    $Script:UserOU = $PSToolsConfig."$Domain".TestServiceAccountOU
                    $Script:AcctEnv = "TST"
                    $Script:SAGroup = $PSToolsConfig."$Domain".TestServiceAccountGroup
                }
                { 'D', 'DEV' -contains $_ } {
                    $Script:UserOU = $PSToolsConfig."$Domain".DevServiceAccountOU
                    $Script:AcctEnv = "DEV"
                    $Script:SAGroup = $PSToolsConfig."$Domain".DevServiceAccountGroup
                }
                { 'S', 'SBX' -contains $_ } {
                    $Script:UserOU = $PSToolsConfig."$Domain".SandboxServiceAccountOU
                    $Script:AcctEnv = "SBX"
                    $Script:SAGroup = $PSToolsConfig."$Domain".SandboxServiceAccountGroup
                }
            } # End Switch
        ...
        foreach ($Account in $Accounts) {
            ...
            $UserProperties = @{
                ...
                EmailAddress         = "$($Account.UserName)@$Domain"
                Path                 = $Account.UserOU
            ...
            try {
                ...
                if ($PSCmdlet.ShouldProcess($Account.UserName, "Add to service account groups")) {
                    ...
                    if ($Account.AzureSync) {
                        Add-ADGroupMember -Server $DomainController -Identity $PSToolsConfig."$Domain".NoMFAGroup -Members $Account.UserName
                    }
                }
