function New-User {
  ...
  process {
    ...
    function Exit-Function {
      ...
      if ($Parameters.Keys -contains 'OutVariable') {
        ...
        if ($FailedUsers.Count -gt 0) {
          ...
          Write-Host "Failed users written to file: $FailedUsersCsv" -ForegroundColor Yellow
        }
      }
    } # End Exit-Function
    ...
    if (-not (Get-Module -Name Az -ListAvailable)) {
      ...
      Write-Host 'Required module Az not installed.' -ForegroundColor Yellow
      break
    }
    ...
    if (-not (Get-Module -Name ActiveDirectory -ListAvailable)) {
      #Handle-Error -e [System.Exception]::new('Required module Az not installed.')
      Write-Host 'Required module ActiveDirectory not installed.' -ForegroundColor Yellow
      break
    }
    ...
    if ($InvalidUsers.Count -gt 0) {
      if ($PSBoundParameters.Keys -contains 'UserFile') {
        Write-Host "There are data issues found in the data file $UserFile." -ForegroundColor Red
        Write-Host "Please check the flagged data:`n" -ForegroundColor Red
      }
      else {
        Write-Host "There are data issues with the provided parameters." -ForegroundColor Red
        Write-Host "Please check the data:`n" -ForegroundColor Red
      }
    ...
    foreach ($User in $Users) {
      ...
      Write-Host "Processing user $($User.FirstName) $($User.LastName)"  -ForegroundColor Green
      ...
      if (-not [string]::IsNullOrWhiteSpace($User.ADGroups)) {
        ...
        Do {
          try {
            ...
            Write-Host "Could not find user to add groups, adding to failed users." -ForegroundColor Red
            $User | Add-Member -MemberType NoteProperty -Name 'Issue' -Value $_.Exception.Message
            $FailedUsers += $User
            break
          }
          ...
          foreach ($Group in $User.ADGroups) {
            if (-not [string]::IsNullOrEmpty($Group)) {
              try {
                ...
                Write-Host "Unable to add user to $Group, adding to failed users."  -ForegroundColor Red
                $User | Add-Member -MemberType NoteProperty -Name 'Issue' -Value $_.Exception.Message
                $FailedUsers += $User
                # Exit group loop
                break
    ...
    Write-Host 'Syncing with AzureAD' -NoNewline -ForegroundColor Green
    # Setup variables
    $Start = Get-Date
      ...
      foreach ($Group in $User.AADGroups) {
        if (-not [string]::IsNullOrEmpty($Group)) {
          try {
            ...
            Write-Host "Unable to add user $($User.UserPrincipalName) to $Group in Azure AD." -ForegroundColor Red
            Write-Host "You will need to add this group manually." -ForegroundColor Red
          }
        }
      }
